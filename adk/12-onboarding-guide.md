...
**Audience:** New team members joining an ADK project. No prior ADK knowledge assumed.

---

## 1. The Big Picture: What ADK Does

ADK (Agent Development Kit) is Google's Python framework for building **multi-agent AI systems**. Think of it as a runtime that connects LLMs, tools, and conversation state into a coherent pipeline.
...
There are **six building blocks** you need to understand. Here's how they fit together:
...
The simplest possible ADK agent:
...
Plain functions become tools automatically:
...
**How the LLM uses tools (the reason-act loop):**
...
Tools can access session state, artifacts, and more through `ToolContext`:
...
**How ToolContext flows through the system:**
...
**Important:** The `tool_context` parameter is detected by its type annotation and automatically excluded from the function declaration sent to the LLM. The LLM never sees it.
...
ADK provides hooks at every layer. Think of them like middleware:
...
Here's a realistic customer support agent system:
