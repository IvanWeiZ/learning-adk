...
**Audience:** ADK developers looking for answers to specific design and architecture questions.
...
│  │                                                                │    │
│  │  Visible to: ALL users, ALL sessions, ALL agents               │    │
│  │  Lifetime:   As long as the app exists                         │    │
│  │  Persisted:  Yes                                               │    │
│  │  Use case:   Global config, feature flags, counters            │    │
│  │                                                                │    │
│  │  ┌────────────────────────────────────────────────────────┐   │    │
│  │  │  USER STATE (prefix: "user:")                           │   │    │
│  │  │                                                         │   │    │
│  │  │  Visible to: ONE user, ALL their sessions               │   │    │
│  │  │  Lifetime:   As long as user data exists                │   │    │
│  │  │  Persisted:  Yes                                        │   │    │
│  │  │  Use case:   Preferences, profile, history              │   │    │
│  │  │                                                         │   │    │
│  │  │  ┌──────────────────────────────────────────────────┐  │   │    │
│  │  │  │  SESSION STATE (no prefix)                        │  │   │    │
│  │  │  │                                                   │  │   │    │
│  │  │  │  Visible to: ONE session only                     │  │   │    │
│  │  │  │  Lifetime:   Duration of this conversation        │  │   │    │
│  │  │  │  Persisted:  Yes                                  │  │   │    │
│  │  │  │  Use case:   Cart, context, current workflow      │  │   │    │
│  │  │  │                                                   │  │   │    │
│  │  │  │  ┌────────────────────────────────────────────┐  │  │   │    │
│  │  │  │  │  TEMP STATE (prefix: "temp:")               │  │  │   │    │
│  │  │  │  │                                             │  │  │   │    │
│  │  │  │  │  Visible to: Current invocation only        │  │  │   │    │
│  │  │  │  │  Lifetime:   In-memory, never persisted     │  │  │   │    │
│  │  │  │  │  Persisted:  NO                             │  │  │   │    │
│  │  │  │  │  Use case:   Auth tokens, cache, scratch    │  │  │   │    │
│  │  │  │  └────────────────────────────────────────────┘  │  │   │    │
│  │  │  └──────────────────────────────────────────────────┘  │   │    │
│  │  └────────────────────────────────────────────────────────┘   │    │
│  └──────────────────────────────────────────────────────────────┘    │
...
def demonstrate_state_scopes(tool_context: ToolContext) -> str:
    """Shows all four state scopes in action."""

    # ─── SESSION STATE (no prefix) ────────────────────────
    # Visible: only in this conversation
    # Persisted: yes (saved to session storage)
    # Shared: no (this session only)
    tool_context.state["cart"] = ["item1", "item2"]
    tool_context.state["last_search"] = "Tokyo flights"
    tool_context.state["step"] = 3

    # ─── USER STATE (prefix: "user:") ─────────────────────
    # Visible: across ALL sessions for this user
    # Persisted: yes
    # Shared: same user, different conversations
    tool_context.state["user:name"] = "Alice"
    tool_context.state["user:preferences"] = {"lang": "en", "theme": "dark"}
    tool_context.state["user:total_orders"] = 42

    # ─── APP STATE (prefix: "app:") ───────────────────────
    # Visible: ALL users, ALL sessions
    # Persisted: yes
    # Shared: entire application
    tool_context.state["app:maintenance_mode"] = False
    tool_context.state["app:model_version"] = "2.5"
    tool_context.state["app:total_requests"] = 100000

    # ─── TEMP STATE (prefix: "temp:") ─────────────────────
    # Visible: this invocation only (single request)
    # Persisted: NO (lost after request completes)
    # Shared: no
    tool_context.state["temp:auth_token"] = "eyJhb..."
    tool_context.state["temp:cache_key"] = "abc123"
    tool_context.state["temp:start_time"] = 1710000000
...
agent = Agent(
    name="support",
    instruction="""You are a support agent for {app:company_name}.
    The customer's name is {user:name}.
    Their subscription tier is {user:tier}.
    They are currently on step {step} of the workflow.

    Maintenance mode is {"on" if {app:maintenance_mode} else "off"}.
    """,
)
```

```
At runtime, ADK resolves:
┌──────────────────────────────────────────────────────────────┐
│                                                                │
│  Template: "Hello {user:name}, your tier is {user:tier}"       │
│                    │                         │                  │
│                    ▼                         ▼                  │
│  state["user:name"] = "Alice"   state["user:tier"] = "premium" │
│                    │                         │                  │
│                    ▼                         ▼                  │
│  Result:  "Hello Alice, your tier is premium"                  │
│                                                                │
└──────────────────────────────────────────────────────────────┘
```
...
async def my_before_agent(callback_context):
    ...
    is_maintenance = callback_context.state.get("app:maintenance_mode", False)

    if is_maintenance and user_tier != "enterprise":
        # Short-circuit: return a maintenance message
        return types.Content(
            role="model",
            parts=[types.Part(text="System is under maintenance. Please try later.")],
        )

    return None

# In a plugin:
class TierPlugin(BasePlugin):
    async def before_model_callback(self, callback_context, llm_request, **kwargs):
        tier = callback_context.state.get("user:tier", "free")
        if tier == "free":
            # Limit context size for free users
            if llm_request.contents and len(llm_request.contents) > 10:
                llm_request.contents = llm_request.contents[-10:]
        return None
```
...
tool_context.state["name"] = "Alice"        # Session-scoped
tool_context.state["user:name"] = "Alice"   # User-scoped (different key!)

name = tool_context.state.get("name")       # Gets session "name", NOT user:name

# ❌ Mistake 2: Expecting temp: to survive across requests
# Request 1:
tool_context.state["temp:token"] = "abc"
# Request 2:
tool_context.state.get("temp:token")  # → None (gone!)

# ❌ Mistake 3: Storing non-serializable objects
tool_context.state["user:db_conn"] = db_connection  # Will crash on persist!

# ✅ Correct: Store serializable data, use right scope for right lifetime
tool_context.state["cart"] = ["item1"]              # Session: this conversation
tool_context.state["user:prefs"] = {"lang": "en"}   # User: all conversations
tool_context.state["app:version"] = "2.0"            # App: everyone
tool_context.state["temp:cache"] = expensive_result  # Temp: this request only
```
